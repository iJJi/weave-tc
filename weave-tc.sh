...
#
# Using iptables, we mark 0x100/0x100 the UDP traffic destined to port $DNS_PORT, that have the
...
iptables -A POSTROUTING -t mangle -p udp --dport $DNS_PORT -m string -m u32 --u32 "28 & 0xF8 = 0" --hex-string "|00001C0001|" --algo bm --from 40 -j MARK --set-mark 0x100/0x100
